...
class Node2Vec:
    ...
    def fit(self, **skip_gram_params) -> gensim.models.Word2Vec:
        ...
        size_key = 'size' if gensim_version < '4.0.0' else 'vector_size'
        ...
        # 创建 Word2Vec 模型实例，但不传递语料库
        model = gensim.models.Word2Vec(**skip_gram_params)

        # 构建词汇表
        model.build_vocab(self.walks)

        # 现在进行训练
        model.train(self.walks, epochs=model.epochs, total_examples=model.corpus_count)
        return model